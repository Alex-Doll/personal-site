...
function Music({ data }) {
    ...
    return (
	      <Layout>
            ...
            <section>
                <Heading as='h2' sx={{ marginTop: 5, marginBottom: 4 }}>Albums ({data.allMarkdownRemark.totalCount}):</Heading>
                {albums.map(album => (
                    <Card key={album.id} sx={{ marginBottom: 5 }} variant='list'>
                        ...
                        <Text variant='caps'><em>{album.frontmatter.date}</em></Text>
                        <Text
                            variant='default'
                            sx={{
                                marginTop: 3,
...
export const query = graphql`
    query {
        allMarkdownRemark(filter: {fields: {slug: {regex: "/\\/music\\/albums\\//"}}}, sort: { fields: [frontmatter___date], order: DESC }) {
            nodes {
                id
